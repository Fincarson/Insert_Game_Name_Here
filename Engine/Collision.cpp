...
bool Collision::IsCollision(int x, int y, const Map& map) {
    ...
    if (map.isBorderTile(top,    left))   return false;
    if (map.isBorderTile(top,    right))  return false;
    if (map.isBorderTile(bottom, left))   return false;
    if (map.isBorderTile(bottom, right))  return false;
